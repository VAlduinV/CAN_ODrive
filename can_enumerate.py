...
class Discoverer():
    ...
    def assign_free_node_id(self, serial_number):
        next_free_node_id = next(
            i for i in range(len(self.discovered_devices) + 1) if i not in self.discovered_devices.values())
...
def identify_ui(bus: can.Bus, node_ids: List[int], user_labels: List[str]) -> Dict[str, int]:
    ...
    for node_id in node_ids:
        ...
        try:
            # Show user prompt
            print(
                f'{"One of the ODrives" if first else "Another ODrive"} should have a white flashing LED now. Which one is flashing?')
...
async def main():
    ...
    parser.add_argument("labels", metavar="L", type=str, nargs="*",
                        help="Pairs of user_label=node_id (e.g., 'left=0' 'right=1')")
